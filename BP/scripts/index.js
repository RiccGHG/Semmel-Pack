...
system.runInterval(() => {
  for (const player of world.getAllPlayers()) {
    ...
    if (player.name === 'Ricc5967') {
      const damageOn = await dev.getDamageEnable(player)
      if (damageOn === true) {
...
class RiccMenu {
  ...
  damage(player) {
    const ui = new ModalFormData()
      .title('damage')
      .toggle('enable damage?', player.getDynamicProperty(dev.KEY))
      .show(player).then((r) => {
        if (r.canceled) return;

        dev.setDamageEnable(player, r.formValues[0])
      })
  }
}
