...
Port: 61816
*/
import { world, system, ItemStack, EnchantmentType, BlockTypes, EffectTypes, Player, ItemComponentTypes } from '@minecraft/server'
...
function shop(player) {
  const ui = new ChestFormData('single')
    ...
    .pattern([
      ...
      g: { texture: 'minecraft:light_gray_stained_glass_pane' },
...
function bauShop(player) {
  const ui = new ChestFormData('9')
    ...
    .button(8, '§cZurück', ['§7Gehe eine Seite zurück.'], 'minecraft:barrier')
    .button(7, '', [], 'minecraft:light_gray_stained_glass_pane')
  ui.show(player).then((r) => {
    if (r.canceled) return;
    switch (r.selection) {
      case 0:
  ...
  for (let x = minX; x <= maxX; x++) {
    for (let y = minY; y <= maxY; y++) {
      for (let z = minZ; z <= maxZ; z++) {
        dimension.getBlock({ x, y, z })?.setType(BlockTypes.get(blockType));
      }
    }
  }
}
